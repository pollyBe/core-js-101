...
function isInsideCircle(circle, point) {
  return (
    Math.hypot(circle.center.x - point.x, circle.center.y - point.y) <
    circle.radius
  );
}
...
function isCreditCardNumber(ccn) {
  return (
    (String(ccn)
      .split('')
      .reverse()
      .reduce((checker, number, i) => {
        if (i % 2) {
          const pre = parseInt(number, 10) * 2;
          if (String(pre).length > 1) {
            return checker + pre - 9;
          }
          return checker + pre;
        }
        return checker + parseInt(number, 10);
      }, 0) *
      9) %
      10 ===
    0
...
function isBracketsBalanced(str) {
  ...
  function del() {
    ...
    while (k < arr.length - 1) {
      if (
        firstMassive.indexOf(arr[k]) !== -1 &&
        secondMassive.indexOf(arr[k + 1]) !== -1 &&
        firstMassive.indexOf(arr[k]) === secondMassive.indexOf(arr[k + 1])
      ) {
        arr.splice(k, 2);
      } else {
        k += 1;
...
function evaluateTicTacToePosition(position) {
  ...
  const diagonals = Array(2)
    .fill(0)
    .map(() => Array(position.length).fill(0));
  diagonals[0] = diagonals[0].map((symb, i) => position[i][i]);
  diagonals[1] = diagonals[1].map(
    (symb, i) => position[i][position.length - i - 1]
  );
